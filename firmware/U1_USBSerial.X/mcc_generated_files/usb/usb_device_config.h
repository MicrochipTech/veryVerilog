...
#define CDC_DATA_INTF_ID        (CDC_COMM_INTF_ID + 1)
#define CDC_DATA_EP             (CDC_COMM_EP + 1)
...
#define CDC_LINE_CONTROL_DESCRIPTOR_SIZE (sizeof(USB_INTERFACE_DESCRIPTOR)+sizeof(USB_CDC_HEADER_FN_DSC)+sizeof(USB_CDC_ACM_FN_DSC)+sizeof(USB_CDC_UNION_FN_DSC)+sizeof(USB_CDC_CALL_MGT_FN_DSC)+sizeof(USB_ENDPOINT_DESCRIPTOR))
#define CDC_DATA_IF_DESCRIPTOR_SIZE (sizeof(USB_INTERFACE_DESCRIPTOR)+sizeof(USB_ENDPOINT_DESCRIPTOR)+sizeof(USB_ENDPOINT_DESCRIPTOR))
#define CDC_DESCRIPTOR_SIZE (CDC_LINE_CONTROL_DESCRIPTOR_SIZE+CDC_DATA_IF_DESCRIPTOR_SIZE)

//Custom confiuration
#define USB_CDC_SET_LINE_CODING_HANDLER USBCDC_baudRateChangeRequest
/* DSR reporting settings */
#define USB_CDC_SUPPORT_DSR_REPORTING
#define mInitDTSPin() /* Init done by MCC */
#define USB_CDC_DSR_IS_ACTIVE (0)
#define USB_CDC_RI_IS_ACTIVE (0) /* Copy DSR value to RI */
#define USB_CDC_CD_IS_ACTIVE (0)

#include "../../icsp_app.h"
#define USB_CDC_SUPPORT_DTR_SIGNALING 
#define USB_CDC_SUPPORT_DTR_SIGNALING_ACTIVATED() do { ICSP_MCLR_keepLow(); } while (0)  // Keep target MCLR low
#define USB_CDC_SUPPORT_DTR_SIGNALING_DEACTIVATED() do { ICSP_MCLR_resetCycle(); } while (0) // Release MCLR after timeout
#define mInitDTRPin() do { } while (0)

/* HID */
#define HID_INTF_ID             (CDC_DATA_INTF_ID + 1)
#define CUSTOM_DEVICE_HID_EP    (CDC_DATA_EP + 1)
#define HID_INT_OUT_EP_SIZE     64
#define HID_INT_IN_EP_SIZE      64
#define HID_NUM_OF_DSC          1
#define HID_RPT01_SIZE          (29+8)
#define HID_INTERFACE_DESCRIPTOR_OFFSET    (sizeof(USB_CONFIGURATION_DESCRIPTOR)+CDC_DESCRIPTOR_SIZE+sizeof(USB_INTERFACE_DESCRIPTOR)) // offset from start of the configuration descriptor to the start of the HID class specific descriptor

#define USER_GET_REPORT_HANDLER USBHID_UserGetReportHandler
#define USER_SET_REPORT_HANDLER USBHID_UserSetReportHandler
