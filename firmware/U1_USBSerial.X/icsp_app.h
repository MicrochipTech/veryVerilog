...
extern "C" {
    ...
    typedef enum PACKED {
        ...
        ICSP_MCMD_LVP_ENTER = 5, // Enter ICSP LVP mode // If parameter==0xFE01, use LSB first LVP enter. Does not reverse the command and data bit order. This has to be done by PC
        ...
        ICSP_MCMD_GPIO = 12, // GPIO (parameter low byte->(primary/alternative function), parameter high byte->LAT; reply: PORT->parameter low byte, parameter high, remains as-is) bit4|5 output disables HWUART. [bit7=RA5,bit6=RA4,bit5=RC5,bit4=RC4,bit3=RC3,bit2=RC2,bit1=RC1,bit0=RC0]
        ICSP_MCMD_DELAY_ms = 13, // Immediate delay for <parameter16> msec. Delay will be interrupted on reception of new HID Write packet. On interrupt ,no reply, new packet will be processed from the first newly received command
    } ICSP_metacommands_e;
