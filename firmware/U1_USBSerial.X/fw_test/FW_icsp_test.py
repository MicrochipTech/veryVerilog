...
class ICSP_HID:
    ...
    def read_word(self, advance_pc = False) -> int:
        ...
        if reply[0] == (0xFE if advance_pc else 0xFC):
    ...
    def blink_rx_tx(self):
        reply = self.xchg_command_block(
            self.get_command_bytes(cmd=12, param=0b1000000011000000, meta_cmd=True) +   #GPIO cmd
            self.get_command_bytes(cmd=13, param=200, meta_cmd=True) + #Delay_ms
            self.get_command_bytes(cmd=12, param=0b0100000011000000, meta_cmd=True) +   #GPIO cmd
            self.get_command_bytes(cmd=13, param=200, meta_cmd=True) + #Delay_ms
            self.get_command_bytes(cmd=12, param=0b1000000011000000, meta_cmd=True) +   #GPIO cmd
            self.get_command_bytes(cmd=13, param=200, meta_cmd=True) + #Delay_ms
            self.get_command_bytes(cmd=12, param=0b0100000011000000, meta_cmd=True) +   #GPIO cmd
        ...
        reply = self.xchg_command_block(
            ...
            self.get_command_bytes(cmd=12, param=0b1000000011000000, meta_cmd=True) +   #GPIO cmd
            self.get_command_bytes(cmd=13, param=200, meta_cmd=True) + #Delay_ms
            self.get_command_bytes(cmd=12, param=0, meta_cmd=True)    #GPIO cmd → all to default function
        )
...
def autodetect_minfpga_path(product_string: str = 'miniFPGA board') -> str | None:
    for device_dict in hid.enumerate():
        ...
        elif device_dict.get('vendor_id')== 0x4d8:
            print(f'Microchip HID device {device_dict.get("product_string")} - {device_dict.get('path')}')
    return None
...
if __name__ == '__main__':
    ...
    if hid_device_path:
        ...
        print(f'@0x0000={pic_icsp.read_word(advance_pc=True)>>1:#08x}')
        print(f'@0x0001={pic_icsp.read_word(advance_pc=True)>>1:#08x}')
        print(f'@0x0002={pic_icsp.read_word(advance_pc=True)>>1:#08x}')
        print(f'@0x0003={pic_icsp.read_word()>>1:#08x}')
        print('DIA →')
        ...
        print('Blinking RX and TX leds')
        pic_icsp.blink_rx_tx()
        hid_device.close()
    else:
