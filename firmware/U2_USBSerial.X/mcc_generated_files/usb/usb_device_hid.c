...
#ifndef HID_INTERFACE_DESCRIPTOR_OFFSET
#define HID_INTERFACE_DESCRIPTOR_OFFSET (9+9) // Usually HID INTERFACE descriptor preceeded by USB_CONFIGURATION_DESCRIPTOR(9bytes)+USB_INTERFACE_DESCRIPTOR(9bytes))
#endif
...
void USBCheckHIDRequest(void)
{
    ...
    if(SetupPkt.bRequest == USB_REQUEST_GET_DESCRIPTOR)
    {
        switch(SetupPkt.bDescriptorType)
        {
            case DSC_HID: //HID Descriptor
                ...
                {
                    USBEP0SendROMPtr(
                        (const uint8_t*)&configDescriptor1 + HID_INTERFACE_DESCRIPTOR_OFFSET,		//It is the offset from start of the configuration descriptor to the start of the HID class specific descriptor.
    ...
    {
        case GET_REPORT:
            #ifdef USER_GET_REPORT_HANDLER
                // Get FEATURE
                USER_GET_REPORT_HANDLER();
            #endif
            break;
        case SET_REPORT:
            #ifdef USER_SET_REPORT_HANDLER 
                // Set FEATURE
